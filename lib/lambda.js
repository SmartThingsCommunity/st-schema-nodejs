...
function createResponse(interactionType, requestId) {
  switch(interactionType) {
    ...
    case 'callbackRequest':
      return "error interaction type"// debug('callbackRequest is not yet implemented');
    default:
      return "error interaction type"// debug(interactionType + ' is invalid');
  }
}
...
module.exports = function(opts) {
  return async function(event, context) {
    ...
    try {
      ...
      console.error("ERROR IN LAMBDA.JS HELPER", err);
      context.fail("Something failed in the lifx viper connector. Blame Vinay!")
    }
